...
class Finanzas():
    ...
    def __convert_2_numeric_type(self, data):
        ...
        # En caso de que se haya escrito el decimal usando coma, intentamos remplazar por punto y convertir
            try:
                converted_data = int(data.replace(',', '.'))
    ...
    def process_file(self, file, separator):
        ...
        # TODO
        #  Me hubiese gustado comprobar también el formato/extensión del fichero de entrada,
        #  pero no me ha dado tiempo a esto último.
        ...
        try:
            ...
            for i in range(0,12):
                if df.columns[i].lower() != self._months[i]:
                    raise InvalidColumnNameOrOrder
            ...
            print(f"Exception: {e}. {file} could not be loaded. Check that the filename is correct.")

        except InvalidNumberOfColumns as e:
            print(f"Invalid number of columns. Expected 12 found {df.shape[1]}.")

        except InvalidColumnNameOrOrder as e:
            print('''Column name or column order is not correct.\n
                  Check the column order and if the months are correctly localized in spanish (es_ES)''')
